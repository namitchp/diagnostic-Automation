...
const SwitchApp = (props) => {
  const isAuthorisedByUrl = new URLSearchParams(props.location.search).get(
    "token"
  );
  const dispatch = useDispatch();
  const getDecryptUserDetailsResponse = useSelector(
    (state) => state.auth.decryptUserDetails
  );
  const checkAuthOtpResponse = useSelector(
    (state) => state.auth.getOtpResponse
  );
  ...
  useEffect(() => {
    if (
      !(
        Object.keys(checkAuthOtpResponse).length === 0 &&
        checkAuthOtpResponse.constructor === Object
      )
    ) {
      if (checkAuthOtpResponse.valid) {
        ...
        localStorage.setItem(
          "encrypt_userId",
          checkAuthOtpResponse.encrypt_userId
        );
        window.location = "/";
      }
    }
